...
for (( i=0; i<IMAGE_COUNT; i++ )); do
  ...
  CAPTION=$(jq -r ".segments[$i].caption" "$PAYLOAD_FILE" | sed "s/'/\\\'/g")

  ffmpeg -loglevel error -y -loop 1 -t $DURATION -i "$IMG" \
    -vf "zoompan=z='zoom+0.0005':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=1:fps=$FPS,
         scale=${WIDTH}:${HEIGHT},
         drawtext=font='$FONT':text='$CAPTION':fontsize=42:fontcolor=white:borderw=1:bordercolor=black:x=(w-text_w)/2:y=h-80,
         format=yuv420p" \
